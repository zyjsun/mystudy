<template>
  <div class="goods">
      <div class="menu-wrapper" ref="menuWrapper">
          <ul>
              <li class="menu-item" 
               v-for="(items,index) in goods" :key="index"  :class="{'current':currentIndex===index}" @click="select(index)">
               <!-- 动态添加类名是否相等 -->
                  <span class="text">
                     <icon :size="1" v-if="items.type>-1" :type="items.type"></icon>{{items.name}}
                  </span>
              </li>
          </ul>
      </div>
...
export default {
    ...
    created(){
        getGoods().then(res=>{
            console.log(res)
            this.goods=res
             this.$nextTick(()=>{//这个函数一定会在页面渲染完成后才执行
                 this._initScroll()
